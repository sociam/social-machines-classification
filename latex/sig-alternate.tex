...
Looking at the set of social machines in a polyarchy leads to a broad/specific relationship emerging that lets us talk about behaviours at various levels of granularity.  We propose looking at nested machines with ``The Web'' as one of several potential roots, with the next level down consisting of sub-platforms (such as Facebook, Twitter, MediaWiki, Ushahid platforms) that spawn more specific social machines. A resulting polyarchy is shown in Figure \ref{polyarchy}, which classifies levels as Infrastructure, Frameworks, Services and Causes/Groups.
...
Finally, we identified several contracts that are relevant for all social machines. First, there are construct that relate to ``analytics'' that can be measured at a point in time, and may change over time. Specifically:

\begin{enumerate}
\item Size
\item Maturity
\item Competitors
\item Diversity of user base
\item Popularity over time
\item Frequency of participation
\end{enumerate}

Next, there are constructs regarding the design affordances of the social machine:

\begin{enumerate}
\item Does it afford anonymity
\item Does it afford quality control or moderation, and in what ways
\item What types of contribution does it afford
\item Openness of resulting data set
\end{enumerate}

In the middle we have a mixture of the result of how the design affordances actually were realised by the social computation, such as how users co-opted / incorporated / interpreted the constraints and: 

\begin{enumerate}
\item worked within them
\item worked around them
\item gamed them
\item extended and enhanced them
\item derived new structures and conventions (e.g., hashtags)
\end{enumerate}

The result is what we call `participation' constructs including:

\begin{enumerate}
\item Roles emerging in the social machine  (domain expert, troll/spammer, hub, personal gainer)
\item Quality of contributions
\item Content of contributions
\item Global / cultural reach
\item Task reach --- what do people use it for? 	
\item Active/Passive participation roles 
\end{enumerate}
